...
"""
Taken from https://github.com/Trax-air/swagger-tester; we will
revert to using the upstream version once/if they've accepted our PRs
"""
...
def validate_definition(swagger_parser, valid_response, response):
    ...
    if valid_response == '' or valid_response is None:
        if type(response) == int:
            logger.warn('Got an int back; this is not strictly kosher but the spec '
                        'for the history server was designed before any attempt to '
                        'make it properly RESTful.')
        assert response is None or response == '' or type(response) == int
        return
...
def swagger_test_yield(swagger_yaml_path=None, app_url=None, authorize_error=None,
    ...
    for action in ['post', 'get', 'put', 'patch', 'delete']:
        for operation in operation_sorted[action]:
            ...
            logger.info(u'TESTING {0} {1} with {2}'.format(action.upper(), url, body))
